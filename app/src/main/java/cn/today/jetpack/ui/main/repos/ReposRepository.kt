...
class ReposRepository @Inject constructor(
        remote: RemoteRepoDataSource,
        local: LocalReposDataSource
) : BaseRepositoryBoth<RemoteRepoDataSource, LocalReposDataSource>(remote, local) {
    ...
    fun fetchRepoPager(): Pager<Int, Repo> {
        ...
        return Pager(config = globalPagingConfig,
                remoteMediator = remoteMediator,
                pagingSourceFactory = { localDataSource.fetchRepoPagingSource() })
    }
}

class RemoteRepoDataSource @Inject constructor(private val serviceManager: ServiceManager) :
        IRemoteDataSource {
...
class RepoPageRemoteMediator(
        private val remoteDateSource: RemoteRepoDataSource,
        private val localDatasource: LocalReposDataSource,
        private val sortKeyProvider: () -> String,
        private val username: String) : RemoteMediator<Int, Repo>() {
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Repo>): MediatorResult {
        return try {
            ...
            val data = remoteDateSource.queryRepos(username, pageIndex, PAGING_REMOTE_PAGE_SIZE, sortKey)
